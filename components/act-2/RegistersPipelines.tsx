...
export default function RegistersPipelines() {
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h3 className="text-xl font-semibold text-slate-900">Registers & Pipelines Diagnostics</h3>
        <p className="mt-2 text-sm text-slate-600">A pipeline stall is often caused by a data hazard when an instruction depends on the result of a previous stage.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2 rounded-3xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="text-sm text-slate-600">Read the pipeline stages from left to right and identify when the next instruction can safely proceed.</p>
        </div>
      </div>
