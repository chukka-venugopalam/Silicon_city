...
export default function SpeculativeLoad() {
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h3 className="text-xl font-semibold text-slate-900">Speculative Load Bypass Diagnostics</h3>
        <p className="mt-2 text-sm text-slate-600">Speculative loads can execute before older stores if dependency checks pass.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2 rounded-3xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="text-sm text-slate-600">Allow speculative loads only when you can confirm there is no address overlap.</p>
        </div>
      </div>
