...
export default function OutOfOrderExecution() {
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h3 className="text-xl font-semibold text-slate-900">Out-of-Order Execution Diagnostics</h3>
        <p className="mt-2 text-sm text-slate-600">Out-of-order issue requires the processor to preserve program order at commit time.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2 rounded-3xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="text-sm text-slate-600">Resolve dependencies before dispatching results back to the architectural register file.</p>
        </div>
      </div>
