...
export default function DMAChannels() {
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h3 className="text-xl font-semibold text-slate-900">DMA Channels Diagnostics</h3>
        <p className="mt-2 text-sm text-slate-600">DMA transfers move data without CPU intervention once the bus is granted.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2 rounded-3xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="text-sm text-slate-600">Issue the transfer and let the DMA controller handle the memory reads and writes.</p>
        </div>
      </div>
