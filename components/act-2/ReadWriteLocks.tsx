...
export default function ReadWriteLocks() {
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h3 className="text-xl font-semibold text-slate-900">Read/Write Locks Diagnostics</h3>
        <p className="mt-2 text-sm text-slate-600">Multiple readers can share a lock, but a single writer needs exclusive access.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2 rounded-3xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="text-sm text-slate-600">Allow concurrent read access only when no writer holds the lock.</p>
        </div>
      </div>
